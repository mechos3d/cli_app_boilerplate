...
RSpec.describe ThisRepo::CliEntrypoint do
  ...
  before do
    ...
    # allow(ThisRepo::MainProcessor).to receive(:call).and_return([])
    # allow(ThisRepo::OutputReportGenerator).to receive(:call)
  end
  ...
  context 'when argument is a relative path to directory' do
    ...
    it 'calls main processor' do
      class_call
      expect(ThisRepo::MainProcessor).to have_received(:call).with(
        urls: expected_urls,
  ...
  context 'when input file doesn\'t exist' do
    ...
    it 'exits with non-zero exit_code(1) and logs to STDERR' do
      allow(ThisRepo::Logger).to receive(:log_error)
      class_call
      aggregate_failures do
        expect(ThisRepo::Logger).to have_received(:log_error).with('No such file or directory')
